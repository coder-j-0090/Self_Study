...
int cmd_seek(int argcnt, char *argvec[]);
...
dispatch_t dispatchTable[] = {
	...
	{"seek", cmd_seek, "Adjust file position: <file> <offset> <whence>"},
...
int cmd_seek(int argcnt, char *argvec[]) {
    if (argcnt != 4) {
        printf("Usage: seek <file> <offset> <whence>\n");
        return -1;
    }

    // Parse arguments
    char *filename = argvec[1];
    int offset = atoi(argvec[2]);
    int whence = atoi(argvec[3]);

    // Open the file
    int fd = b_open(filename, O_RDWR);
    if (fd < 0) {
    ...
    // Perform the seek
    int new_position = b_seek(fd, offset, whence);
    if (new_position < 0) {
        printf("Error: Seek operation failed.\n");
    } else {
        printf("Seek successful. New position: %d\n", new_position);
    }
